...
Do not call the CodexEngine + browser-harness integration complete because one
surface works. Completion requires all three proof layers below.

### 1. Product Contract Proof

Every user-facing entry point must enter the same agent path:

- TUI task, followup, queued followup, cancel, history, and completed output.
- CLI `run-openai` and dataset-run paths.
- Python SDK `Client.agent.run` for real providers.
- Fake/offline SDK tests may keep the deterministic in-memory path.

For each Browser Use product feature, record the exact product-to-harness
mapping before marking it complete:

| Feature | Required proof |
| --- | --- |
| `/browser` | Stored backend becomes CodexEngine `BrowserMode` and harness env before turn start. |
| `/profile` | Stored profile id/label becomes harness local/cloud profile intent before turn start. |
| `/sync-cookies` | Cloud profile/cookie intent is handed to the harness manager, or explicitly scoped out for eval. |
| `/domains` | Domain allow/block policy is available to the harness/browser layer without asking the model to enforce it. |
| `/secrets` and `/import-passwords` | Secrets are available through the runtime/harness secret path and are redacted from public events. |
| `/email` | Disposable inbox config is available to the harness/browser layer and visible through model-safe helpers only. |
| `/model` | Provider/model choice is fixed before Codex thread start and is recorded in run metadata. |
| `/goal` | Native goal support is either wired through Codex or explicitly left as Browser Use store/UI state. |

### 2. Local Verification Proof

The exact commit to be evaluated must pass:
...
For TUI changes, inspect `/tmp/but-design-loop/` and keep the verifier log path
in this tracker. Unit tests and dump screens are not enough.

### 3. Eval Proof

The evaluated run must be generated by `scripts/run-internal-bench-hard-openai.sh`
from the same commit that passed local verification.

Required properties:

- dataset hash matches `/home/exedev/datasets/Internal_Bench_hard.json`
- `simple_harness=true`
- `codex_engine=true`
- `disable_local_search=true`
- `browser_mode=cloud`
- `MAX_TURNS >= 10000`
- 106 selected task ids for the full run
- 106 runner packets
- 106 native event logs
- 106 judge packets
- 106 judgments
- completion audit passes with `--require-judged`
- raw-reference comparison is produced against the recorded raw Codex +
  browser-harness aggregate

Canonical eval sequence:

```bash
# 1. Prove the wrapper is pointed at the benchmark path without launching.
scripts/run-internal-bench-hard-openai.sh \
  --dry-run \
  --judge \
  --run-id ibh-dry-current \
  --root /tmp/ibh-dry-current

# 2. Optional focused smoke after product-path changes.
scripts/run-internal-bench-hard-openai.sh \
  --judge \
  --run-id ibh-smoke-current \
  --root /home/exedev/eval-runs/ibh-smoke-current \
  --task-id zcotoh \
  --task-id 82kkzm \
  --task-id v18kgy \
  --task-id swebnv \
...
# 3. Full judged Internal_Bench_hard.
scripts/run-internal-bench-hard-openai.sh \
  --judge \
  --run-id ibh-full-current \
  --root /home/exedev/eval-runs/ibh-full-current

# 4. Re-run the hard completion audit explicitly before reporting.
scripts/audit-ibh-run-completion.py \
  --run-root /home/exedev/eval-runs/ibh-full-current \
  --run-id ibh-full-current \
  --expected-total 106 \
  --require-judged
```

If Claude print-mode judging is unavailable, the run is not fully judged until
the five prepared packet chunks have equivalent locked-rubric judgments and
`scripts/finalize-ibh-judged-run.sh` plus the completion audit both pass.
...
- current-tree dry-run after SDK/TUI product contract patch:
  - `scripts/run-internal-bench-hard-openai.sh --dry-run --judge --run-id ibh-dry-current-contract --root /tmp/ibh-dry-current-contract`
  - result: `EXPECTED_TOTAL=106`, `TASK_IDS=all`, command includes
    `-c simple_harness=true`, `-c codex_engine=true`,
    `-c disable_local_search=true`, `--model gpt-5.5`, `--max-turns 10000`,
    `--concurrency 25`, `--browser-mode cloud`, and the judge command uses
    five locked packet chunks.
- current-tree dry-run after Codex-login provider/default-model fix:
  - `scripts/run-internal-bench-hard-openai.sh --dry-run --judge --run-id ibh-dry-current-post-provider-fix --root /tmp/ibh-dry-current-post-provider-fix`
  - result: still `EXPECTED_TOTAL=106`, `TASK_IDS=all`, and the command still
    uses `simple_harness=true`, `codex_engine=true`,
    `disable_local_search=true`, `gpt-5.5`, `--max-turns 10000`,
    `--concurrency 25`, `--browser-mode cloud`, plus the five-chunk judge.
- post-checkpoint dry-run:
  - `scripts/run-internal-bench-hard-openai.sh --dry-run --judge --run-id ibh-dry-c683a5c-contract --root /tmp/ibh-dry-c683a5c-contract`
  - result: same 106-task OpenAI/cloud/simple-harness/CodexEngine command and
    five-chunk judge command.
...
- [x] One non-browser task works through TUI with a live model.
- [x] One SDK `Agent.run` works and streams events.
...
- API-key provider fix: Browser Use OpenAI defaults map to custom Codex
  ...
  overrides.
- SDK smoke:
  - `uv run --with pytest python -m pytest -q python/tests/test_browser_use_sdk.py`
  - result: `19 passed` in the focused run before the full TUI verifier.
  - `scripts/verify-terminal-ui.sh` later ran the repo Python test set as part
    of the terminal definition of done.
- live TUI Codex-login smoke after product contract/provider-id fixes:
  - state: `/tmp/but-live-tui-codex-proof4`
  - terminal capture:
    `/tmp/but-design-loop/live-tui-codex-proof4-screen.txt`
  - event capture:
    `/tmp/but-design-loop/live-tui-codex-proof4-events.json`
  - prompt: `Reply with exactly TUI_CODEX_ENGINE_LIVE_OK and nothing else.`
  - result: `session.done` with `{"result":"TUI_CODEX_ENGINE_LIVE_OK","source":"codex_app_server"}`.
  - event proof: `codex_engine.started`, `codex.thread.started`,
    `codex.turn.started`, `model.stream_delta`, `model.turn.response`,
    `session.done`, and `agent.completed` are present; no `session.failed` or
    `agent.failed`.
  - fix found by this smoke: Browser Use `Codex login` now maps to Codex
    app-server provider id `openai`, because the embedded Codex provider table
    has no provider named `codex`.
  - adjacent CLI cleanup: `run-codex`, `run-codex-session`, and
    `dataset-run-codex` default to `gpt-5.5`, matching the TUI Codex-login
    default and avoiding the rejected `gpt-5.1-codex` ChatGPT-account model.

### Gate 2: TUI/SDK Contract Preserved

- [x] `task tui:dev` starts the real existing TUI.
- [x] TUI task submission uses CodexEngine.
- [x] TUI active followup works.
- [x] TUI queued followup works.
- [x] TUI cancel works.
- [x] TUI history shows completed CodexEngine runs.
- [x] SDK JSON-RPC API shape is unchanged.
- [x] Python SDK `Client.agent.run` uses CodexEngine for real providers.
...
- Full agent-lib status/runtime regression suite:
  `cargo test -p browser-use-agent --lib`.
- Product contract patch after the `94/106` eval:
  - SDK real-provider `agent.run` now routes through `RuntimeAgentExecutor`,
    which is the same path that owns CodexEngine preparation, Codex app-server
    turns, simple-harness setup, mirroring, cleanup, and child-agent runners.
  - SDK fake provider remains on the deterministic in-memory driver so Python
    tests stay offline and stable.
  - New regression anchor:
    `cargo test -p browser-use-cli sdk_real_providers_route_through_codex_executor -- --nocapture`.
  - TUI browser-option tests now assert `codex_engine=true` alongside
    `simple_harness=true` for Local Chrome, Browser Use Cloud, managed headed,
    and managed headless modes.
  - `task tui:dev -- --state-dir /tmp/but-tui-dev-contract-check --dump-screen`
    starts `target/debug/but` and renders the setup screen.
  - `scripts/verify-terminal-ui.sh` passed after this patch.
  - Artifact directory inspected: `/tmp/but-design-loop/`.
  - Real terminal smoke captures include followup, queued input, cancel/escape,
    history switching, slash palette, transcript scrollback, paste/resize, and
    completed plain-output flows.
...
- [x] Harness root/workspace is explicit and under Browser Use artifacts.
- [x] Harness worker socket is explicit.
- [x] Run/session identity is explicit through session-scoped harness paths and
      `BU_NAME`.
- [x] `/browser` setting feeds browser-harness manager intent.
- [x] `/profile` setting feeds browser-harness manager intent.
- [x] `/sync-cookies` cloud profile intent feeds browser-harness manager.
- [x] SDK `browser.set_backend` feeds the same intent.
- [x] SDK `browser.set_profile` feeds the same intent.
- [x] Browser manager cleanup runs on done/fail/cancel.
- [x] Public browser evidence excludes secrets, CDP URLs, and provider browser
      IDs.

Gate 3 verification:

- `SimpleHarnessPaths::from_session(...)` derives `home`, `agent-workspace`,
  `runtime_dir`, `tmp`, worker socket, worker log, worker events, `events.jsonl`,
  and `final.txt` from Browser Use session metadata and artifact root.
- `SimpleHarnessPaths::harness_env()` injects `CODEX_HOME`,
  `BROWSER_USE_HARNESS_HOME`, `BH_AGENT_WORKSPACE`,
  `BH_DOMAIN_SKILLS_ROOT`, `BH_RUNTIME_DIR`, `BH_TMP_DIR`,
  `BU_HARNESS_WORKER_SOCKET`, `BU_AUTOSPAWN`, and session-derived `BU_NAME`.
- Browser/profile intent is carried with `BUT_BROWSER_MODE`,
  `BUT_BROWSER_PROFILE_ID`, `BUT_BROWSER_PROFILE_LABEL`,
  `BUT_BROWSER_LOCAL_BROWSER`, `BU_AUTOSPAWN_PROFILE_ID`, and
  `BU_AUTOSPAWN_PROFILE_NAME`.
- `/sync-cookies` remains a product-side operation, not a model-visible
  browser helper. The TUI stores the synced `cloud_profile.id` into
  `browser.preference.profile` and the label into
  `browser.preference.profile_label`; for cloud browser runs,
  `SimpleHarnessPaths::apply_to_config(...)` converts those stored preferences
  into `BU_AUTOSPAWN_PROFILE_ID` / `BU_AUTOSPAWN_PROFILE_NAME`; the generated
  browser-harness bootstrap passes `BU_AUTOSPAWN_PROFILE_ID` as
  `profileId` to `start_remote_daemon(...)`.
- Regression coverage:
  - `simple_harness::tests::apply_to_config_sets_harness_python_env`
  - `simple_harness::tests::apply_to_config_uses_cloud_profile_name_when_id_absent`
  - `simple_harness::tests::browser_harness_cloud_bootstrap_forwards_selected_profile_id`
  - `simple_harness::tests::apply_to_config_carries_local_profile_without_cloud_autospawn`
  - `simple_harness::tests::browser_harness_worker_redacts_resolved_secret_output`
  - `onboarding_cookie_sync_completes_with_default_model_after_cloud_profile`
  - `sdk_json_rpc_browser_settings_feed_stored_defaults_to_run_config`
  - `sdk_provider_run_config_maps_browser_use_options_to_rust_core`

### Gate 4: Required Codex Fork Support

- [x] Lossless raw event path exists or audited runs fail on event lag.
- [x] Thread/session shell env is not process-global.
- [x] Browser-harness skill text/root is thread-scoped or safely injected.
- [x] Codex auth/config home is separate from browser-harness runtime state.
- [x] Provider fixed-per-session behavior is documented, or per-turn provider
      switching is implemented.
- [x] Auth refresh behavior is tested or explicitly out of scope for eval path.

Gate 4 verification:

- Codex app-server raw notifications are persisted as `codex.raw_event` /
  `codex.raw_response_item.completed`; event stream lag is fatal for audited
  runs. Regression coverage:
  `cargo test -p browser-use-codex-engine`.
- Shell/browser-harness environment is carried on `ProviderRunConfig.options`
  and per-session harness paths, not by mutating process-global env.
- Browser-harness skill text is written into the per-session `CODEX_HOME` under
  the artifact root.
- `CODEX_HOME` points at the session harness home, while Browser Use runtime
  state remains in `BUT_STATE_DIR`.
- Provider selection is fixed per run/session through `model_provider_id` and
  the CodexEngine thread start config; per-turn switching is intentionally not
  part of the eval path.
- Auth refresh is out of scope for the Internal_Bench_hard eval path: the
  benchmark wrapper uses the OpenAI API-key provider (`browser_use_openai_api`)
  and Browser Use Cloud key, not ChatGPT/Codex OAuth token refresh. The
  in-process app-server client explicitly rejects
  `ChatgptAuthTokensRefresh`, so long-running ChatGPT-account refresh remains a
  product hardening item outside the OpenAI-key benchmark proof.
...
- Durable verifier log:
  `/tmp/but-design-loop/verify-terminal-ui-20260619-contract.log`.
- Post Codex-login provider/default-model fix verifier log:
  `/tmp/but-design-loop/verify-terminal-ui-20260619-after-codex-provider-fix.log`.
- Artifact directory inspected: `/tmp/but-design-loop/`.
...
- Current commit eval preflight after product contract/provider fixes:
  - command:
    `scripts/run-internal-bench-hard-openai.sh --judge --run-id ibh-full-c683a5c-preflight-<timestamp> --root /home/exedev/eval-runs/ibh-full-c683a5c-preflight-<timestamp>`
  - result: launcher stopped before task execution because this shell/store has
    no OpenAI API key and no Browser Use Cloud key.
  - missing auth reported by wrapper:
    `OPENAI_API_KEY` or `LLM_BROWSER_OPENAI_API_KEY`, and
    `BROWSER_USE_API_KEY`.
  - dry-run immediately before this preflight proves the eval command shape is
    correct; a fresh judged run can start once these credentials are available
    through env or `auth login`.
