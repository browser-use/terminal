...
#[derive(Debug, Clone, Serialize)]
pub struct LocalChromeDebuggingDisabled {
    pub browser_name: String,
    pub user_data_dir: PathBuf,
    pub browser_running: bool,
    /// `Some(false)` is "user revoked", `None`
    /// is "never granted but Chrome is running so the user is about to need
    /// to grant".
    pub user_enabled: Option<bool>,
}

#[derive(Debug, Clone, Default, Serialize)]
pub struct OpenLocalChromeDebuggingSetupResult {
    pub opened: bool,
    pub error: Option<String>,
}

/// Return every running Chromium-family install we'd attach to with Local
/// Chrome. Include installs where `user-enabled` is `Some(true)` — that
/// flag only controls whether CDP is permitted at all, NOT whether Chrome
/// shows its per-session "Allow remote debugging?" prompt. In Chrome 136+
/// that prompt fires on every new CDP connection regardless of `user-enabled`,
/// so the TUI heads-up modal needs to fire on every fresh attach.
///
/// Installs where Chrome isn't running are skipped: there's no CDP attach to
/// warn about until the user opens Chrome, and the agent will fail with a
/// different error if Local Chrome is selected but no process is up.
pub fn local_chrome_attach_candidates() -> Vec<LocalChromeDebuggingDisabled> {
    known_local_browser_installs()
        .into_iter()
        .filter_map(|install| {
            let browser_running =
            ...
            let user_enabled = remote_debugging_user_enabled(&install.user_data_dir);
            Some(LocalChromeDebuggingDisabled {
                browser_name: install.browser_name,
                user_data_dir: install.user_data_dir,
                browser_running,
                user_enabled,
...
/// the picker should only nudge the user when something actually needs
/// fixing, not on every config change; the per-attach heads-up is handled by
/// [`local_chrome_attach_candidates`] at agent-start time.
pub fn check_local_chrome_debugging_disabled() -> Vec<LocalChromeDebuggingDisabled> {
    known_local_browser_installs()
        .into_iter()
        .filter_map(|install| {
            let user_enabled = remote_debugging_user_enabled(&install.user_data_dir);
            let browser_running =
                browser_process_running(&install.browser_name, &install.browser_path)
                    .unwrap_or(false);
            let needs_setup = match user_enabled {
                Some(false) => true,
                None => browser_running,
                Some(true) => false,
            };
            if !needs_setup {
                return None;
...
/// Open `chrome://inspect/#remote-debugging` directly in Chrome (or the
/// closest Chromium-family install we can find on disk) so the user can
/// toggle "Allow remote debugging for this browser instance" on.
///
/// `chrome://` is an internal scheme — `open::that` / `xdg-open` typically
/// can't route it because no app is registered for it at the OS level. We
/// have to invoke the browser binary itself with the URL as an argument,
/// otherwise the call fails with the cryptic "Launcher … failed with
/// ExitStatus" error the user reported.
///
/// `browser_name_hint` lets the caller bias selection toward a specific
/// install (e.g. the one the user is currently running). When the hint
/// matches a known install we use that; otherwise we fall back to the first
/// `Google Chrome` install on disk, then any install.
///
/// We deliberately do NOT quit, restart, or edit Chrome — pre-setting
/// `user-enabled=true` doesn't bypass Chrome 136+'s per-session CDP
/// permission prompt, and quitting kills the user's tabs.
pub fn open_local_chrome_debugging_setup(
    ...
    let target = browser_name_hint
        ...
        .or_else(|| {
            installs
                .iter()
                .find(|install| install.browser_name == "Google Chrome")
        })
        .or_else(|| installs.first());

    if let Some(install) = target {
        match open_chrome_internal_url(&install.browser_name, &install.browser_path, URL) {
            Ok(()) => {
                return OpenLocalChromeDebuggingSetupResult {
                    opened: true,
                    error: None,
                };
            }
    ...
    // No known Chrome install — last-resort attempt at the OS handler. This
    // almost certainly fails for `chrome://` URLs, but the caller will fall
    // back to "open it yourself" copy on failure.
    match open::that(URL) {
        Ok(()) => OpenLocalChromeDebuggingSetupResult {
            opened: true,
            error: None,
        },
        Err(error) => OpenLocalChromeDebuggingSetupResult {
            opened: false,
            error: Some(format!("{error:#}")),
        },
    }
...
#[cfg(target_os = "macos")]
fn open_chrome_internal_url(browser_name: &str, _browser_path: &Path, url: &str) -> Result<()> {
    // `open -a <App Name> <url>` hands the URL off to the app's own URL
    // handler — that's how chrome:// links work end-to-end on macOS.
    let status = Command::new("open")
        .args(["-a", browser_name, url])
        .status()
        .context("invoke `open -a` to load chrome:// URL")?;
    if !status.success() {
        bail!("`open -a {browser_name}` exited with status {status}");
    }
    Ok(())
}

#[cfg(not(target_os = "macos"))]
fn open_chrome_internal_url(_browser_name: &str, browser_path: &Path, url: &str) -> Result<()> {
    // Linux/Windows: invoke the binary directly with the URL. Chrome'll route
    // it to an existing window if one is open, otherwise spawn fresh.
    Command::new(browser_path)
        .arg(url)
        .spawn()
        .with_context(|| format!("spawn {} with chrome:// URL", browser_path.display()))?;
    Ok(())
}
