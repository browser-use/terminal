...
Source-discovery contract: for research, lookup, comparison, document, pricing, list, or "find the source" tasks, first consider `search` or `web_search` to identify likely official/source URLs, then use `read_url` for public/static result pages or browser/CDP/fetch when interaction, cookies, or visual verification are needed. Do not spend repeated browser navigation steps on broad search result pages when a search tool can cheaply find candidate URLs.
