...
Bounded verification contract: after a large result is saved, verify it with a compact independent check of count/schema/source coverage and at most one targeted repair pass. Do not keep restarting full crawls or pagination sweeps when the source is unstable or intermittently returns different counts. If the best saved artifact is the strongest available evidence, finalize from it with its count/schema/source notes instead of timing out in repeated audits.
...
When the turn budget is nearly exhausted, stop starting new lines of investigation. Finalize from the strongest current evidence only as an explicitly incomplete fallback, write any partial artifacts, and name unknown or ambiguous fields instead of timing out with no deliverable.
